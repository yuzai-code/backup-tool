...
type BackupService interface {
	BackupService(id int)
}

// PathRepositoryImpl 实现了 PathRepository 接口
type PathRepositoryImpl struct {
	pathRepo repository.PathRepository
	logger   *zap.Logger
}

// NewPathRepositoryImpl 创建一个新的 PathRepositoryImpl 实例
func NewPathRepositoryImpl(pathRepo repository.PathRepository, logger *zap.Logger) *PathRepositoryImpl {
	return &PathRepositoryImpl{pathRepo: pathRepo, logger: logger}
}

// BackupService 处理备份服务的逻辑
func (p *PathRepositoryImpl) BackupService(id int) {
	// 根据id获取需要备份的文件的信息
