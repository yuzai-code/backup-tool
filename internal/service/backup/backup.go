...
func (p *BackupServiceImpl) BackupService(id int) error {
	...
	if err != nil {
		// 处理错误
		logger.Log.Error("获取文件信息失败", zap.Error(err))
		return err
	}
	...
	if err := os.MkdirAll(savePath, os.ModePerm); err != nil {
		logger.Log.Error("创建目录失败", zap.Error(err))
		return err
	}
	...
	if isCompressedFile(srcFilePath) {
		logger.Log.Info("文件路径", zap.String("srcFilePath", srcFilePath))
		// 直接复制文件
		if err := copyFile(srcFilePath, dstFilePath); err != nil {
			logger.Log.Error("复制文件失败", zap.Error(err))
			return err
		}
	} else {
		// 压缩文件
		if err := compressFile(srcFilePath, dstFilePath); err != nil {
			logger.Log.Error("压缩文件失败", zap.Error(err))
			return err
		}
	}

	logger.Log.Info("文件备份成功", zap.String("srcFilePath", srcFilePath), zap.String("dstFilePath", dstFilePath))
	return nil
}
...
func copyFile(srcFilePath, dstFilePath string) error {
	...
	if err != nil {
		logger.Log.Error("获取源文件信息失败", zap.Error(err))
	...
	if err != nil {
		logger.Log.Error("打开源文件失败", zap.Error(err))
		return fmt.Errorf("打开源文件失败: %w", err)
	}
	defer srcFile.Close()

	// 创建目标文件
	logger.Log.Info("准备创建目标文件", zap.String("dstFilePath", dstFilePath))
	dstFile, err := os.Create(dstFilePath)
	if err != nil {
		logger.Log.Error("创建目标文件失败", zap.Error(err))
	...
	if err != nil {
		logger.Log.Error("复制文件内容失败", zap.Error(err))
		return fmt.Errorf("复制文件内容失败: %w", err)
	}

	logger.Log.Info("文件复制完成",
		zap.String("srcFilePath", srcFilePath),
